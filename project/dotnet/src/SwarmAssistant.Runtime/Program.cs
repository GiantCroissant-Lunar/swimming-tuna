...
builder.Services.AddSingleton<ArcadeDbSwarmRunRepository>();
builder.Services.AddSingleton<ISwarmRunWriter>(serviceProvider => serviceProvider.GetRequiredService<ArcadeDbSwarmRunRepository>());
builder.Services.AddSingleton<ISwarmRunReader>(serviceProvider => serviceProvider.GetRequiredService<ArcadeDbSwarmRunRepository>());
builder.Services.AddSingleton<ArcadeDbTaskExecutionEventRepository>();
builder.Services.AddSingleton<ITaskExecutionEventWriter>(serviceProvider => serviceProvider.GetRequiredService<ArcadeDbTaskExecutionEventRepository>());
        ...
        CancellationToken cancellationToken) =>
    {
        ...
        try
        {
            ...
            var dtos = result.Agents.Select(entry => new AgentRegistryEntryDto(
                ...
                Status: AgentStatusResolver.ResolveAgentStatus(entry.Budget, entry.ConsecutiveFailures),
