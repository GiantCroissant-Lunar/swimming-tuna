    ...
    public static string BuildPrompt(ExecuteRoleTask command, StrategyAdvice? strategyAdvice)
    {
        var basePrompt = command.Role switch
        {
            SwarmRole.Planner => string.Join(
                ...
                "If the task involves UI/UX design, you MUST first run the UI/UX Pro Max skill search script to establish a design system: `python3 .claude/skills/ui-ux-pro-max/scripts/search.py \"context\" --design-system --persist -p \"ProjectName\"`",
            ...
            SwarmRole.Builder => string.Join(
                ...
                "If you are building A2UI/GenUI components (GenUiComponent), you MUST use the design system generated by the ui-ux-pro-max skill. Read the design rules from `design-system/MASTER.md` (and any page-specific overrides in `design-system/pages/`). Apply the semantic colors (as `fontColor`, `bgColor`), typography (`fontSize`), and spacing rules directly to the GenUiComponent properties.",
