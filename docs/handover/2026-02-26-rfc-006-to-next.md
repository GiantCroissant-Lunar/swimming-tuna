...
#### Key files to modify
- `project/dotnet/src/SwarmAssistant.Runtime/Actors/TaskCoordinatorActor.cs` — query sibling output before builder dispatch
- `project/dotnet/src/SwarmAssistant.Runtime/Planning/RolePromptFactory.cs` — add sibling context layer (7th)
- `project/dotnet/src/SwarmAssistant.Runtime/Actors/DispatcherActor.cs` — dependency-aware task ordering
- `project/dotnet/src/SwarmAssistant.Runtime/Actors/InternalMessages.cs` / contracts — add `DependsOn` to TaskAssigned
- `project/dotnet/src/SwarmAssistant.Runtime/Endpoints/A2AEndpoints.cs` — accept `dependsOn` in task submission
